...
\section{Motivation}
\noindent The motivation for this thesis is the constant need for improvement on the
students learning management process and the teachers overview of each students
struggles and progress. The main goal is to integrate the current concept of
the Learning Scorecard into the ISCTE Moodle Environment that is configured
yearly, allowing for seamless data collection and metric calculations. This
integration with the ISCTE Moodle means that, for example, all quiz, exercise
and personal data that is already present can be reused, as in previous
Learning Scorecard versions, without the need of an external application and
manual input by either the faculty or the students.

This migration will also provide a more stable, scalable and secure Learning
Scorecard that's built on top of this solution. It will use the ISCTE moodle
infrastructure (apache server and database) without the need of an external
connection or another server.
\section{Statement of the Problem}
\noindent The obstacles with the current solution for the Learning Scorecard are based on the need for manual input by the students and faculty and the constant need to validate, check and visit an external site in which to access the LS.\@ This also comes with the added concerns of security and data protection aswell as the need for external infrastructure that would increase the running costs of the Learning Scorecard.

\section{Aims and Objectives}
\noindent The aim of this thesis is to provide a proof of the feasibility of a Learning Scorecard integrated with the ISCTE Moodle environment and attempt to achieve a seamless integration that would provide the LS with everything it needs from the Moodle database, reducing the manual input to the minimum possible.
To achieve this, I will attempt to conceptualize a system architecture that benefits my goals without pushing back on the progress done previously by my colleagues, provide a minimum viable product (MVP) that proves the feasibility of the system and serves as a demonstration and a base setup on the Learning Scorecard 2.5.
...
In the latest version of the Learning Scorecard, version 2.0, the technological
stack consisted of a React~\cite{react_dev} frontend that communicated with an
Express~\cite{express_js} (NodeJS~\cite{nodejs_org}) backend, the backend, in
turn communicated through REST with a virtual machine hosted in
Azure~\cite{azure_microsoft} to a NoSQL mongoDB~\cite{mongodb_database}
database and a graphDB~\cite{graphdb_ontotext}. The main platform was hosted in
...
This architecture, although scalable, modular and modern, has a few flaws,
mainly in the fact that it is not integrated with the ISCTE moodle environment
(second functional requirement), relying on manual input by the faculty and
students making it not an ideal system and one that would become more of a
chore for teachers who already have so much to grade, register and manage and
students who have multiple classes, quizes and exercises to keep track of.
...
services, although it is less secure to have students information on a
independent and standalone website and would require the web services to be
enabled and for the Learning Scorecard system to have a private API key for
authentication. It would also be reliant on the students and faculty accessing
an external platform which would require an incentive to do so from the
teacher. Heroku's free tier where a developer could host a website for free has
also since been discontinued~\cite{heroku_pricing} and would incur an extra
payment to keep the Learning Scorecard running.

A more viable option would be to create a set of moodle plugins which are
installed into the ISCTE moodle environment yearly. Moodle is extremely modular
and has available a service API that provides almost all functionality needed
to retrieve, update and expand the students information, grades and provide
gamification features.

We can satisfy the requirements 1,2 and 3 just with the new LS version
integrated as a moodle plugin. The first requirement specifies the necessity of
a profile and authentication credentials. Every student receives a moodle
account aswell as authentication credentials at the start of each new course
year. The second requirement is also already satisfied as the LS 2.5 version
will be a plugin of moodle, therefore it will be integrated with the e-learning
system by itself. Lastly, the third requirement we can already satisfy is also
a base moodle functionality. All accounts are user accounts although their role
(student, teacher, etc) limits what they can access.

The proof of concept for this Learning Scorecard transition to a set of moodle
plugins will be what I will be exploring throughout this thesis as it seems
like the most integrated, affordable and scalable option. It will run in the
ISCTE servers, attached to the already present and yearly updated moodle
instance. This way, we can achieve a seemless integration for the students and
faculty without the costs and security issues that would arise from the
standalone LS web application.
